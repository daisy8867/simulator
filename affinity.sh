...
do
	...
	do
		...
		awk -v s=start 'BEGIN{use=0} {if(NR%2) {use=1;} else {use=-1} print $1 " " use}' $a/gputime | tee -a affinity/$k/gputime 
		rm $a/report
	...
	mv io_* affinity/$k
	rm affinity/$k/tmp
	rm affinity/$k/plotgputime
	cat affinity/$k/gputime | sort > affinity/$k/tmp
	start=$(awk 'BEGIN{getline; print $2}' affinity/$k/io_affinity)
	awk -v s=$start 'BEGIN{use=0; print 0 " " 0;} {use+=$2; print ($1-s)/100000  " " use}' affinity/$k/tmp | tee -a affinity/$k/plotgputime
gnuplot << EOF
	set term pdfcairo font "Times New Roman, 15" size 9cm, 3cm
	set output "affinity/$k/gpuusage.pdf"
	...
	plot "affinity/$k/plotgputime"  with steps lw 2 lt 24 lc  rgb "#DC143C"  notitle 
	set output
EOF
done
