...
do
	...
	do
		...
		awk -v s=start 'BEGIN{use=0} {if(NR%2) {use=1;} else {use=-1} print $1 " " use}' $a/gputime | tee -a fifo/$k/gputime 
		rm $a/report
	...
	mv io_* fifo/$k
	rm fifo/$k/tmp
	rm fifo/$k/plotgputime
	cat fifo/$k/gputime | sort > fifo/$k/tmp
	start=$(awk 'BEGIN{getline; print $2}' fifo/$k/io_fifo)
	awk -v s=$start 'BEGIN{use=0; print 0 " " 0;} {use+=$2; print ($1-s)/100000  " " use}' fifo/$k/tmp | tee -a fifo/$k/plotgputime
gnuplot << EOF
	set term pdfcairo font "Times New Roman, 15" size 9cm, 3cm
	set output "fifo/$k/gpuusage.pdf"
	...
	plot "fifo/$k/plotgputime"  with steps lw 2 lt 24 lc  rgb "#DC143C"  notitle 
	set output
EOF
done
