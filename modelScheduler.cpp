...
void modelScheduler(vector<App>& apps, Node& n, const vector<Model>& model) {
...
//	}
	...
	function<bool (const App&, const App&)> cmpCPU = [&](const App& a, const App& b) {
		return a.num_cpu > b.num_cpu;
	};
	atomic<int> io_mutex = ATOMIC_VAR_INIT(0);
	launchContainers.emplace_back(monitor, boost::ref(n), "io_model", boost::ref(io_mutex));
	while(!apps.empty()){
