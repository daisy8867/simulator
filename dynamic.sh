...
do
	...
	do
		...
		awk -v s=start 'BEGIN{use=0} {if(NR%2) {use=1;} else {use=-1} print $1 " " use}' $a/gputime | tee -a dynamic/$k/gputime 
		rm $a/report
	...
	mv io_* dynamic/$k
	rm dynamic/$k/tmp
	rm dynamic/$k/plotgputime
	cat dynamic/$k/gputime | sort > dynamic/$k/tmp
	start=$(awk 'BEGIN{getline; print $2}' dynamic/$k/io_dynamic)
	awk -v s=$start 'BEGIN{use=0; print 0 " " 0;} {use+=$2; print ($1-s)/100000  " " use}' dynamic/$k/tmp | tee -a dynamic/$k/plotgputime
gnuplot << EOF
	set term pdfcairo font "Times New Roman, 15" size 9cm, 3cm
	set output "dynamic/$k/gpuusage.pdf"
	...
	plot "dynamic/$k/plotgputime"  with steps lw 2 lt 24 lc  rgb "#DC143C"  notitle 
	set output
EOF
done
